...
export default function Friends() {
    ...
    const handleClickFollow = async (_user) => {
        try {
            ...
            const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/AddFollow`, {
                method: "POST",
    ...
    const handleGetMoreFriends = async() =>{
        ...
        try {
            const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/${type}`, {
                method: "POST",
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    userId: user._id,
    ...
    const FriendsComponent = ({_user}) => {
        ...
        if(!data) return  
        return (
            <View style={{width:'100%',height:60,backgroundColor:'black',display:'flex',flexDirection:'row',justifyContent:'space-between'}}>
                <View style={{display:'flex',flexDirection:'row',marginLeft:10,gap:10,alignItems:'center'}}>
                    <ExpoImage source={{uri:`${process.env.EXPO_PUBLIC_API_URL}/uploads/${data.profile_img}`}} style={{width:55,height:55,borderRadius:50}}/>
                    <View>
                    <Text style={{color:Color.WHITE,fontSize:18,fontWeight:'bold'}}>{data.username}</Text>
                    <Text style={{color:Color.WHITE,fontSize:16}}>{data.fullName}</Text>
                </View>
                </View>
