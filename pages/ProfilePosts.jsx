...
const ProfilePosts = ({ post,posts,index,setPosts }) => {
  ...
  const HandleShowPreviewPost = () =>{
    navigation.push('FeedProfile', {posts:[post],index:index,setPosts:setPosts});
  }
  return (
    <>
      {isVideo ? (
        <PostContainer $image={post.thumbnail} onPress={HandleShowPreviewPost}>
          <ExpoImage source={{uri:`${process.env.EXPO_PUBLIC_API_URL}/uploads/${post.thumbnail}`}} style={{objectFit:'fill',height:'100%',width:'100%'}} />
        ...
        <PostContainer activeOpacity={.7} onPress={HandleShowPreviewPost}>
          <PostImage source={{uri:`${process.env.EXPO_PUBLIC_API_URL}/uploads/${post.post_imgs[0].data}`}}/>
          <PostsHighlightContainer>
